...
export default class AccountsController extends Controller {
    ...
    remove() {
        let currentUserId = this.HttpContext.path.params.currentUserId;
        let userIdToRemove = this.HttpContext.path.params.userIdToRemove;
        if (this.repository != null) {
            let currentUser = this.repository.findByField("Id", currentUserId);
            let userToRemove = this.repository.findByField("Id", userIdToRemove);
            
            if (currentUser && userToRemove) {
                if (currentUserId === userIdToRemove || (currentUser.Authorizations.readAccess === 3 && currentUser.Authorizations.writeAccess === 3)) {
                    this.repository.remove(userIdToRemove);
                    this.HttpContext.response.accepted("User removed successfully.");
                } else {
                    this.HttpContext.response.unAuthorized("You do not have permission to remove this user.");
                }
            } else {
